...
import detaStorage from "./components/deta_storage/save-load-deta"
...
swal("Nice!", "You wrote: " + inputValue, "success");
  ...
  async function handleTypeAccount() {
    if (config.values["deta-edition"] == false) {
      // BootstrapVue.Modal(document.getElementById('accountLogIn'), options)
      // BootstrapVue.show('myModal');
    }
    else {


      if (config.values["dev-mode"] == true) {
        var props = {
        "package_name": String(document.getElementById("package_name").value),
        "package_author": String(document.getElementById("package_author").value),
        "package_version": String(document.getElementById("package_version").value),
      ...
      props
      var xml = await String(Blockly.Xml.domToPrettyText(Blockly.Xml.workspaceToDom(foo.value.workspace)))
      await detaStorage.save("test", xml, props);


      } else {

        Swal.fire({
    title: "Save into account",
    icon: 'info',
    html: "This feature is currently not avaiable, it will be implemented in the future",
    confirmButtonText: "OK 👍"
  });
      }
    } 
          ...
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu bg-white">
                ...
                <li><a class="dropdown-item" id="save_acc" v-on:click="handleTypeAccount()">Save into account</a></li>
